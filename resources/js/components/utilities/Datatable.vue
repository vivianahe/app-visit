  ...
  <div>
    <v-card :elevation="elevation">
      <v-card-title class="title-datatable-section" v-if="!hide_header">
        <div class="title-datatable">
          ...
          <div class="d-flex flex-wrap div_botones">
            ...
            <v-spacer></v-spacer>
            <v-btn prepend-icon="mdi mdi-plus-circle" class="btn-log" v-if="button_add" @click="$emit('click-add')">
              Agregar
            </v-btn>
            <div v-if="addBtnRole">
              <v-btn prepend-icon="mdi mdi-plus-circle" class="my-2 pull-right btn btn-primary ms-auto"
      ...
      <v-data-table :search="search" loading-text="Cargando ..." :no-data-text="'Listado vacío'" :headers="headers"
        :items-per-page="10" :items="items" @current-items="getFiltered" :show-select="show_select" v-model="itemsSelect" @update:modelValue="changeItems" return-object>
